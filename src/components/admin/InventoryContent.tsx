...
const InventoryContent = () => {
  ...
  useEffect(() => {
    const fetchInventory = async () => {
      try {
        setIsLoading(true);
        const response = await fetch('https://papeleria-back.vercel.app/v2/papeleria/getInventoryProductsapi', {
          headers: {
  ...
  const fetchProducts = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('https://papeleria-back.vercel.app/v2/papeleria/getProductsapi');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('https://papeleria-back.vercel.app/v2/papeleria/assignProductToInventoryapi', {
        method: 'POST',
        headers: {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch('https://papeleria-back.vercel.app/v2/papeleria/deleteInventoryProductapi', {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
