...
const checkAndReserveSaleCode = async (code: string): Promise<boolean> => {
  try {
    const response = await fetch(`https://papeleria-back.vercel.app/v2/papeleria/checkAndReserveSaleCodeapi/${code}`, {
      method: 'POST',
      headers: {
...
const releaseSaleCode = async (code: string): Promise<void> => {
  try {
    await fetch(`https://papeleria-back.vercel.app/v2/papeleria/releaseSaleCodeapi/${code}`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
...
const getLastRegisteredSaleCode = async (): Promise<string | null> => {
  try {
    const response = await fetch('https://papeleria-back.vercel.app/v2/papeleria/getLastSaleCodeapi', {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    });
...
const CartPanel: React.FC<CartPanelProps> = ({ onStockUpdate }) => {
  ...
  const handleCreateSale = async () => {
    ...
    try {
      ...
      const response = await fetch('https://papeleria-back.vercel.app/v2/papeleria/createSaleapi', {
        method: 'POST',
        headers: {
